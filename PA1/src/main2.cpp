...
int main(int argc, char *argv[]) {
	...
	// extract x and y from string line 
	double x = std::stod(str,&sz);
	double y = std::stod(str.substr(sz));
	...
	// convert stirng to point	
	std::string target (name);
	std::string::size_type size;
	// extract x and y from string line 
	double x = std::stod(target,&size);
	double y = std::stod(target.substr(size));
	Point goal = Point(x,y);

        BSTIterator<Point> item = kdtree.findNearestNeighbor(goal);
